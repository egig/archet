...
describe('buildServerBundle', () => {
  ...
  it('mounts /_ratchet, publicDir, and the web router in the bundled server entry when routes/root.tsx exists', async () => {
    ...
    expect(entrySrc).toContain(
      "import { createWebRouter, createWebAssetsRouter, createPublicAssetsRouter } from '@egig/ratchet/web/router';",
    );
    ...
    expect(entrySrc).toContain('createPublicAssetsRouter("public")');
