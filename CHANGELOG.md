...
- `Workspace`'s `locked` is a plain field, not a dedicated operation — freezing a row is a normal `update` write to `locked`, gated by the same `update` permission + `locked` field grant as any other column (superseded a same-cycle, never-released `lock`/`unlock`-as-permission-actions design now generalized by custom operations above).
