 ...
 * server entry into one file. Does NOT change `arche serve`/`arche dev`'s runtime behavior,
...
export async function buildServerBundle(cwd: string, dirs: Dirs): Promise<void> {
  const entrySrc = [
    `// GENERATED by \`arche build\` — do not edit.`,
    `import path from 'node:path';`,
    ...
    `import { createAuthRouter } from 'arche/auth';`,
    `import { createAdminRouter } from 'arche/admin';`,
