...
description: Named actions like `lock`/`unlock`, built from the same pipeline primitives.
---
...
For anything beyond "write these fixed fields," write your own `PipelineFn` — a custom operation composes with [`pipe()`](/docs/pipelines) exactly like `create`/`update`/`remove` do, and can do anything a pipeline function can: call `persist`/`persist.remove`, call an external API, send an email, or not touch the database at all.
