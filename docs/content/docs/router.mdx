---
title: REST API
description: Filtering, sorting, pagination, and `?include=` on the generic `/api/:model` router.
---
...
`ratchet/router` exports the pieces `ratchet serve` composes for you, if you need to mount them yourself (e.g. inside a custom `App`):
